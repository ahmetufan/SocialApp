...
import com.ahmet.socialapp.ViewFullImageActivity
    ...
    override fun onCreateViewHolder(parent: ViewGroup, position: Int): ViewHolder {
        ...
        return if (position == 1) {
            val view: View = LayoutInflater.from(mContext)
                .inflate(com.ahmet.socialapp.R.layout.message_item_right, parent, false)
            ViewHolder(view)
        } else {
            val view: View = LayoutInflater.from(mContext)
                .inflate(com.ahmet.socialapp.R.layout.message_item_left, parent, false)
            ViewHolder(view)
        }
    }
        ...
        if (position == mChatList.size-1) {
           ...
           if (chat.isIsSeen()) {
               ...
               if (chat.getMessage().equals("sent you an image.") && !chat.getUrl().equals("")) {
                   ...
                   val dp : RelativeLayout.LayoutParams? = holder.text_seen!!.layoutParams as RelativeLayout.LayoutParams?
                   dp!!.setMargins(0,245,10,0)
                   holder.text_seen!!.layoutParams=dp
               }
           }
            else {
        ...
        val ref=FirebaseDatabase.getInstance().reference.child("Chats")
            ...
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    ...
                    Toast.makeText(holder.itemView.context, "Silinemedi", Toast.LENGTH_SHORT).show()
                }
            }
    }
}